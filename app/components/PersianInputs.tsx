...
function useExclusivePopover() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("sepah-popover-open", closeOther);
    document.addEventListener("pointerdown", closeOutside);
    return () => { window.removeEventListener("sepah-popover-open", closeOther); document.removeEventListener("pointerdown", closeOutside); };
  }, [id]);
  function toggle() {
    if (!open) window.dispatchEvent(new CustomEvent("sepah-popover-open", { detail: id }));
    setOpen((current) => !current);
  }
  return { root, open, setOpen, toggle };
}
...
export function PersianDateInput({ value, defaultValue, onChange, placeholder = "۱۴۰۵/۰۵/۲۸" }: { value?: string; defaultValue?: string; onChange?: (value: string) => void; placeholder?: string }) {
  const initial = parseDate(value ?? defaultValue); const { root, open, setOpen, toggle } = useExclusivePopover(); const [viewYear, setViewYear] = useState(initial.year); const [viewMonth, setViewMonth] = useState(initial.month);
